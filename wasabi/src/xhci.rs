...
// Address Array is utilize by the xHCI Scratchpad mechanism.
#[repr(C, align(64))]
        ...
        for sb in table.iter_mut() {
            let buf = ALLOCATOR.alloc_with_options(
                Layout::from_size_align(page_size, page_size)
                    .map_err(|_| "could not allocate scratchpad buffer")?,
            );
...
impl EventRing {
    ...
    async fn poll(&mut self) -> Result<()> {
        if let Some(e) = self.pop()? {
            ...
            // cleanup state waiters
            let state_waiter_indices = self
                .wait_list
                .iter()
                .enumerate()
                .rev()
...
impl CommandRing {
    ...
    pub fn push(&mut self, mut src: GenericTrbEntry) -> Result<u64> {
        ...
        // as far as this function doew not move the ring out.
...
impl PortSc {
    fn new(bar: &BarMem64, cap_regs: &CapabilityRegisters) -> Self {
        ...
        for port in 1..=num_ports {
            ...
            // always points to a valid PORTSC entity under the condition.
...
impl Doorbell {
    ...
    pub fn notify(&self, target: u8, task: u16) {
        ...
        // SAFETY: This is safe as logn as the ptr is valid
        unsafe {
