...
impl Header {
    fn can_provide(&self, size: usize, align: usize) -> bool {
        // This check is rought -actual size needed may be smaller.
    ...
    fn provide(&mut self, size: usize, align: usize) -> Option<*mut u8> {
        ...
        if self.is_allocated() || !self.can_provide(size, align) {
            ...
            //                                      ^ header_for_padding
            //                                      ^
            // header_for allocated.end_addr() self has enough space
...
impl Drop for Header {
    fn drop(&mut self) {
        panic!("Header should never be dropped!");
    }
}
...
mod test {
    ...
    #[test_case]
    // malloc_iterate_free_and_alloc は基本的な alloc と free の繰り返しが正しく動作するか確認する
    ...
    #[test_case]
    // malloc_align はアラインメントを大きくしながら alloc してみて正しく動作していることを確認する
    fn malloc_align() {
    ...
    #[test_case]
    // malloc_align_random_order はアラインメントをランダムな順番で alloc してみて正しく動作していることを確認する
    ...
    #[test_case]
    // allocated_objects_have_no_overlap は確保した領域が重複していないことを確認する
